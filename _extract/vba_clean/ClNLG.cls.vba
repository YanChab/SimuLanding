...
Private mBaguePiston As Double
Private mBagueGuide As Double
Private mFRIBagueGuide As Double
Private mFRIBaguePiston As Double
...
For i = 0 To 3
f(0) = md * St - (mVh * mPgtamp / mOilBulk) - xRes(0) - xRes(1) * (Atn(20 * (xRes(2) - mPinithp)) + Pi / 2) * 1 / Pi
...
df(0, 0) = -1: df(0, 1) = -(Atn(20 * (xRes(2) - mPinithp)) + Pi / 2) * 1 / Pi: df(0, 2) = -xRes(1) / Pi * (20 / (1 + (20 * (xRes(2) - mPinithp)) ^ 2))
...
FFriJoi = Sgn(mv) * mCoeffAtte * ((0.0207 * Pd + 110649) * (Application.WorksheetFunction.Pi / 4 * (mASeal * mASeal - mDt * mDt)) + 2 * fc * mDt * Application.WorksheetFunction.Pi)
End If
...
If mv <> 0 Then
mCoeffAtte = 1 / Sqr(0.95 + 0.28 * Sqr(1 / (90 * Abs(mv))))
mFRIBagueGuide = ((-0.0007 * (Abs(mXGt) / (mDt * mBagueGuide * 1000000)) + 0.1248) + 0.0825 * Abs(mv) + 0.0898) / 2
mFRIBaguePiston = ((-0.0007 * (Abs(mXGt) / (mDt * mBaguePiston * 1000000)) + 0.1248) + 0.0825 * Abs(mv) + 0.0898) / 2
FFriBag = Sgn(mv) * (mFRIBaguePiston * Abs(mXGt) + mFRIBagueGuide * Abs(mXGb)) * mCoeffAtte
...
End Property
'''''''' BagueGuide ''''''''''
Property Get BagueGuide() As Double
' Propriété en lecture
BagueGuide = mBagueGuide
End Property
Property Let BagueGuide(BagueGuide As Double)
' Propriété en écriture
mBagueGuide = BagueGuide
End Property
'''''''' BaguePiston ''''''''''
Property Get BaguePiston() As Double
' Propriété en lecture
BaguePiston = mBaguePiston
End Property
Property Let BaguePiston(BaguePiston As Double)
' Propriété en écriture
mBaguePiston = BaguePiston
End Property
